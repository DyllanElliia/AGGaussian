...
__device__ void computeColorFromSH(
  ...
  if (deg > 0) {
    ...
    if (deg > 1) {
      ...
      if (deg > 2) {
        ...
        dRGBdx +=
            (SH_C3[0] * sh[9] * 3.f * 2.f * xy + SH_C3[1] * sh[10] * yz +
  ...
  glm::vec3 dL_ddir(
      glm::dot(dRGBdx, dL_dRGB), glm::dot(dRGBdy, dL_dRGB), glm::dot(dRGBdz, dL_dRGB));
...
__global__ void computeCov2DCUDA(
  ...
  if (denom2inv != 0) {
    ...
    dL_da = denom2inv *
            (-c * c * dL_dconic.x + 2 * b * c * dL_dconic.y + (denom - a * c) * dL_dconic.z);
    dL_dc = denom2inv *
            (-a * a * dL_dconic.z + 2 * a * b * dL_dconic.y + (denom - a * c) * dL_dconic.x);
    dL_db = denom2inv * 2 *
            (b * c * dL_dconic.x - (denom + 2 * b * b) * dL_dconic.y + a * b * dL_dconic.z);
...
__device__ void computeCov3D(
  ...
  *dL_drot =
      float4{dL_dq.x, dL_dq.y, dL_dq.z, dL_dq.w};  // dnormvdv(float4{ rot.x, rot.y, rot.z, rot.w },
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA(
    ...
    const float *__restrict__ dL_dpixel_sem,
  ...
  if (inside) {
    ...
    for (int i = 0; i < SC; ++i) dL_dps[i] = dL_dpixel_sem[i * H * W + pix_id];
  ...
  float last_semantics[SC] = {0};
  float last_depth = 0;

  // -------------------------------------------------------------------------------------
  // for compute gradient with respect to the depth and feature distortion map
  float final_A = 1 - T_final;
  float depth_fin_D = inside ? final_T[pix_id] : 0;
  float depth_fin_D2 = inside ? final_T[pix_id + H * W] : 0;
  float feat_fin_D[SC] = {0}, feat_fin_D2[SC] = {0};
  float dL_dd_dist = 0;
  float dL_df_dist[SC] = {0};
  if (inside) {
    for (int ch = 0; ch < SC; ++ch) {
      feat_fin_D[ch] = final_T[pix_id + (ch + 2) * H * W];
      feat_fin_D2[ch] = final_T[pix_id + (ch + 2 + SC) * H * W];
    }
    dL_dd_dist = dL_ddistortion[pix_id + DEPTH_DISTORTION_OFFSET * H * W];
    for (int i = 0; i < SC; ++i)
      dL_df_dist[i] = dL_ddistortion[pix_id + (i + FEATURE_DISTORTION_OFFSET) * H * W];
  }
  float last_depth_dL_dT = 0, last_feat_dL_dT[SC] = {0};
  // -------------------------------------------------------------------------------------
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE) {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
      ...
      // pair).
      float dL_dopa = 0.0f, dL_dz = 0.0f, dL_dsem_feat[SC] = {0};
      ...
      for (int sch = 0; sch < SC; sch++) {
        const float sl = collected_semantics[sch * BLOCK_SIZE + j];
        // Update last color (to be used in the next iteration)
        accum_recsem[sch] =
            last_alpha * last_semantics[sch] + (1.f - last_alpha) * accum_recsem[sch];
        last_semantics[sch] = sl;

        const float dL_dchannel = dL_dps[sch];
        dL_dopa += (sl - accum_recsem[sch]) * dL_dchannel;
        // Update the gradients w.r.t. color of the Gaussian.
        // Atomic, since this pixel is just one of potentially
        // many that were affected by this Gaussian.
        // atomicAdd(&(dL_dsemantics[global_id * SC + sch]), dchannel_dcolor * dL_dchannel);
        dL_dsem_feat[sch] += dchannel_dcolor * dL_dchannel;
      }
      ...
      dL_dz += dpixel_depth_ddepth * dL_dpixel_depth;
      ...
      // -------------------------------------------------------------------------------------
      // for compute gradient with respect to the depth and feature distortion map
      // Propagate gradients from distortion map to opacity, depth and feature
      float dL_ddwi, dL_dfwi;
      const float m_d = far_n / (far_n - near_n) * (1 - near_n / c_d);
      const float dmd_dd = (far_n * near_n) / ((far_n - near_n) * c_d * c_d);

      dL_ddwi = (depth_fin_D2 + m_d * m_d * final_A - 2 * m_d * depth_fin_D) * dL_dd_dist;
      dL_dopa += dL_ddwi - last_depth_dL_dT;
      // propagate the current weight W_{i} to next weight W_{i-1}
      last_depth_dL_dT = dL_ddwi * alpha + (1 - alpha) * last_depth_dL_dT;
      const float dL_dmd = 2 * (T * alpha) * (m_d * final_A - depth_fin_D) * dL_dd_dist;
      dL_dz += dmd_dd * dL_dmd;

      // for (int sch = 0; sch < SC; ++sch) {
      //   const float m_f = collected_semantics[sch * BLOCK_SIZE + j];
      //   dL_dfwi =
      //       (feat_fin_D2[sch] + m_f * m_f * final_A - 2 * m_f * feat_fin_D[sch]) *
      //       dL_df_dist[sch];
      //   dL_dopa += dL_dfwi - last_feat_dL_dT[sch];
      //   // propagate the current weight W_{i} to next weight W_{i-1}
      //   last_feat_dL_dT[sch] = dL_dfwi * alpha + (1 - alpha) * last_feat_dL_dT[sch];
      //   const float dL_dmf = 2 * (T * alpha) * (m_f * final_A - feat_fin_D[sch]) *
      //   dL_df_dist[sch];
      //   // atomicAdd(&(dL_dsemantics[global_id * SC + sch]), dL_dmf);
      //   dL_dsem_feat[sch] += dL_dmf;
      // }
      ...
      // Update gradients w.r.t. semantic feature of the Gaussian
      for (int sch = 0; sch < SC; sch++) {
        atomicAdd(&(dL_dsemantics[global_id * SC + sch]), dL_dsem_feat[sch]);
      }
      ...
      // Update gradients w.r.t. opacity and depth of the Gaussian
      atomicAdd(&(dL_dopacity[global_id]), G * dL_dopa);
      atomicAdd(&(dL_ddepths[global_id]), dL_dz);
    }
  }
}
