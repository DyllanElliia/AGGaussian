...
CudaRasterizer::GeometryState CudaRasterizer::GeometryState::fromChunk(char *&chunk, size_t P) {
  ...
  // obtain(chunk, geom.semantic, P * SEM_CHANNELS, 128);
...
CudaRasterizer::ImageState CudaRasterizer::ImageState::fromChunk(char *&chunk, size_t N) {
  ImageState img;
  // NOTE: save D and D2 of depth and feature distortion here (2+SegFeatDim*2)
  obtain(chunk, img.accum_alpha, N * (2 + SEM_CHANNELS * 2), 128);
...
int CudaRasterizer::Rasterizer::forward(
  ...
  CHECK_CUDA(
      cudaMemcpy(
          &num_rendered, geomState.point_offsets + P - 1, sizeof(int), cudaMemcpyDeviceToHost),
      debug);
  ...
  CHECK_CUDA(
      FORWARD::render(
          ...
          geomState.means2D,
          feature_ptr,
          semantic_features,  // geomState.semantic,
          geomState.depths,
          geomState.conic_opacity,
